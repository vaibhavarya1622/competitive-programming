...
void solve(){  
    ...
    for(int i=1;i<n;++i){
        ll h = ceil(sqrt(d));
        while(j<i && points[j].first<points[i].first-h){
            s.erase({points[j].second,points[j].first});
            ++j;
        }
        auto l = s.lower_bound({points[i].second-h,0});
        auto r = s.upper_bound({points[i].second+h,0});

        for(auto it = l; it != r; ++it){
            d = min(d,dis({it->second,it->first},points[i]));
        }
